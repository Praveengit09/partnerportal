...
declare var nv: any;
@Component({
    selector: 'averagevisittime',
    templateUrl: './averagevisittime.template.html',
    styleUrls: ['./averagevisittime.style.scss'],
    encapsulation: ViewEncapsulation.None
})
export class AverageVisitTime {
    nvd32Chart: any;
    nvd32Data: any;
    ...
    applyNvd3Data(): void {
        ...
        if (this.reportResponse == undefined || this.reportResponse.length == 0) {
            ...
            coor.y = this.reportResponse[0].averageVisitTime;
            ...
            for (let i = 1; i < this.reportResponse.length; i++) {
                ...
                for (let j = 0; j < num; j++) {
                    if (this.reportResponse[i].pocId == responseList[j].id) {
                        ...
                        coor.y = this.reportResponse[i].averageVisitTime;
        ...
        // console.log("Nvd31Data:: " + JSON.stringify(this.nvd31Data));
        d3.select('#chart08')    //Select the <svg> element you want to render the chart in.   
