...
declare var nv: any;
@Component({
    selector: 'averagequeuetimePharmacyOrders',
    templateUrl: './averagequeuetimePharmacyOrders.template.html',
    styleUrls: ['./averagequeuetimePharmacyOrders.style.scss'],
    encapsulation: ViewEncapsulation.None
})
export class AverageQueueTimePharmacyOrders{
    nvd32Chart: any;
    nvd32Data: any;
    ...
    applyNvd3Data(): void {
        ...
        if (this.reportResponse == undefined || this.reportResponse.length == 0) {
            ...
            coor.y = this.reportResponse[0].averageQueueTimeForPharmacyOrders;
            ...
            for (let i = 1; i < this.reportResponse.length; i++) {
                ...
                for (let j = 0; j < num; j++) {
                    if (this.reportResponse[i].pocId == responseList[j].id) {
                        ...
                        coor.y = this.reportResponse[i].averageQueueTimeForPharmacyOrders;
        ...
        d3.select('#chart09')    //Select the <svg> element you want to render the chart in.   
