...
@Component({
  selector: 'orderperdaypharmacychart',
  templateUrl: './orderperdaypharmacychart.template.html',
  styleUrls: ['./orderperdaypharmacychart.style.scss'],
  encapsulation: ViewEncapsulation.None
})

export class OrderPerdayPharmacyChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getPharmacyRevenueBasedOnDay().then(response => {
      ...
      console.log("Response In  OrderPerdayPharmacyChartComponent NgOnInit:: " + JSON.stringify(this.reportResponse));
      this.applyNvd3Data();
    }).catch((err) => {
      if (err) {
  ...
  onDateOptionChange(index: number) {
    ...
    this.getPharmacyRevenueBasedOnDay().then(response => {
      this.spinnerService.stop();
      this.reportResponse = response;
      console.log("Response In OrderPerdayPharmacyChartComponent OnDateOptionChange:: " + JSON.stringify(this.reportResponse));
      this.applyNvd3Data();
    });
  }
  ...
  applyNvd3Data(): void {
    ...
    function addGraph(): Array<any> {
      ...
      data.push(response);
      console.log("OrderPerdayPharmacyChartComponent In ApplyNvd3Data AddGraph: " + JSON.stringify(response));
      return data;
    }
