...
@Component({
  selector: 'walkinadmissionnote_component',
  templateUrl: './walkinadmissionnote.template.html',
  encapsulation: ViewEncapsulation.Emulated,
  styleUrls: ['./walkinadmissionnote.style.scss']
})
export class WalkinAdmissionNoteComponent implements OnInit, OnDestroy, AfterViewInit {
  admissionnoteAdviseTrack: BasketRequest;
  startdate: Date;
  datepickerOpts = {
  ...
  constructor(private paymentService: PaymentService, private router: Router, private diagnosticsService: DiagnosticsService,
    ...
    this.admissionnoteAdviseTrack = new BasketRequest();
    this.admissionnoteAdviseTrack.cartItemList = new Array<CartItem>();
    this.admissionnoteAdviseTrack.cartItemList[0] = new CartItem();
    this.admissionnoteAdviseTrack.cartItemList[0].pocId = this.pocId;
    this.admissionnoteAdviseTrack.cartItemList[0].bookingSource = this.admissionnoteAdviseTrack.bookingSource = 3;
    this.admissionnoteAdviseTrack.cartItemList[0].paymentSource = 3;
    this.admissionnoteAdviseTrack.cartItemList[0].cartItemType = CartItem.CART_ITEM_TYPE_ADMISSION_NOTE;
    this.pdfHeaderType = auth.userAuth.pdfHeaderType;
    this.admissionnoteAdviseTrack.cartItemList[0].empId = auth.userAuth.employeeId;
  ...
  onFinalSubmit(): void {
    ...
    if (this.admissionnoteAdviseTrack.cartItemList.length > 0) {
      if (this.admissionnoteAdviseTrack.cartItemList[0].serviceList == undefined || this.admissionnoteAdviseTrack.cartItemList[0].serviceList.length <= 0) {
        this.isError = true;
        this.errorMessage = new Array();
      ...
      if (this.admissionnoteAdviseTrack.cartItemList[0].serviceList && this.admissionnoteAdviseTrack.cartItemList[0].serviceList.length > 0) {
        this.admissionnoteAdviseTrack.cartItemList[0].serviceList.forEach(service => {
        ...
        this.admissionnoteAdviseTrack.cartItemList[0].serviceList = this.cartItemList[0].serviceList;
      }
      ...
      this.admissionnoteAdviseTrack.cartItemList[0].doctorDetail = new DoctorDetails();
      this.admissionnoteAdviseTrack.cartItemList[0].doctorDetail.firstName = this.doctorFirstName;
      this.admissionnoteAdviseTrack.cartItemList[0].doctorDetail.lastName = this.doctorLastName;
      this.admissionnoteAdviseTrack.cartItemList[0].doctorId = 0;
      this.admissionnoteAdviseTrack.cartItemList[0].brandId = this.brandId;
      if (this.paymentModeIndex == 1) {
        if (!this.admissionnoteAdviseTrack.transactionType) {
          this.isError = true;
          this.errorMessage = new Array();
        ...
        this.admissionnoteAdviseTrack.cartItemList[0].payment.paymentStatus = PaymentType.PAYMENT_STATUS_PAID;
        this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType;
        this.admissionnoteAdviseTrack.transactionId = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionId = this.transactionId;
      } else if (this.paymentModeIndex == 2) {
        this.admissionnoteAdviseTrack.cartItemList[0].payment.paymentStatus = PaymentType.PAYMENT_STATUS_PAID;
        this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_CASH;
      } else if (this.paymentModeIndex == 5) {
        this.admissionnoteAdviseTrack.cartItemList[0].payment.paymentStatus = PaymentType.PAYMENT_STATUS_PENDING;
        this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_MOBILE;
      }
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType;
      this.admissionnoteAdviseTrack.transactionId = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionId = this.transactionId;
      ...
      this.diagnosticService.initiatePayment(this.admissionnoteAdviseTrack).then(
        response => {
          this.spinnerService.stop();
          if (response != null && response != undefined) {
            this.admissionnoteAdviseTrack = response;
  ...
  onPaymentChange(index: number): void {
    if (index == 1) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_CARD;
    };
    if (index == 2) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_PHONEPE;
    };
    if (index == 3) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_GOOGLE_PAY;
    };
    if (index == 4) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_PAYTM;
    }
    if (index == 11) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_NEFT;
    };
    if (index == 12) {
      this.admissionnoteAdviseTrack.transactionType = this.admissionnoteAdviseTrack.cartItemList[0].payment.transactionType = Payment.PAYMENT_TYPE_UPI;
    }
  ...
  searchTests(key) {
    this.key = key;
    if (key.length > 2) {
      this.paymentService.getSearchedAdmissionNotes(key, this.pocId).then((cartItemAdmissionNotes) => {
        this.cartItemSearchList = cartItemAdmissionNotes;
        this.searchItemsTotal = cartItemAdmissionNotes.length;
        this.searchedTests = cartItemAdmissionNotes;
  ...
  onChangeAmount(): void {
    let tempCart = this.admissionnoteAdviseTrack.cartItemList[0];
    tempCart.serviceList.forEach((item) => {
      item.netPrice = item.grossPrice;
    })
    tempCart.isReset = true;
    this.admissionnoteAdviseTrack.cartItemList[0] = { ...tempCart }
  }
  ...
  addNewTest(): void {
    ...
    let tempCartItem = this.admissionnoteAdviseTrack.cartItemList[0];
    tempCartItem.isReset = true;
    this.admissionnoteAdviseTrack.cartItemList[0] = { ...tempCartItem };
    ...
    this.admissionnoteAdviseTrack.cartItemList[0].serviceList = this.cartItemList[0].serviceList;
    this.otherDiscountAmount = this.admissionnoteAdviseTrack.cartItemList[0].payment.otherDiscountAmount = 0;
  ...
  saveSelectedProfile() {
    this.admissionnoteAdviseTrack.cartItemList[0].patientProfileId = this.selectedRegisteredProfile.selectedProfile.profileId;
    this.admissionnoteAdviseTrack.cartItemList[0].parentProfileId = this.selectedRegisteredProfile.selfProfile.profileId;
    this.admissionnoteAdviseTrack.parentProfileId = this.selectedRegisteredProfile.selfProfile.profileId;
    this.admissionnoteAdviseTrack.cartItemList[0].patientRelationship = this.selectedRegisteredProfile.selectedProfile.relationShip;
  }
  ...
  ngOnDestroy(): void {
    if (this.admissionnoteAdviseTrack != undefined && this.admissionnoteAdviseTrack != null) {
      this.paymentService.admissionnoteAdviseTrack = this.admissionnoteAdviseTrack.cartItemList[0];
    }
  }
