...
@Component({
  selector: 'totalrevenuechartonboarded',
  templateUrl: './totalrevenuechartonboarded.template.html',
  styleUrls: ['./totalrevenuechartonboarded.style.scss'],
  encapsulation: ViewEncapsulation.None
})

export class TotalRevenueChartOnboardedComponent implements OnInit {
  nvd31Chart: any;
  nvd31Data: any;
  ...
  ngOnInit(): void {
    ...
    this.getRevenues().then(response => {
      ...
      console.log("Response In TotalRevenue NgOnInit:: " + JSON.stringify(this.dashBoardChartRespList));
      this.applyNvd3Data();
    }).catch((err) => {
      if (err) {
  ...
  applyNvd3Data(): void {
    ...
    if (this.dashBoardChartRespList == undefined || this.dashBoardChartRespList[0].values.length == 0) {
      ...
      //console.log("HighYAxis in  TotalRevenue:: " + this.highYAxis);
    }
