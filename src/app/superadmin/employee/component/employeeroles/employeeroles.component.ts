...
export class EmployeeRolesComponent implements OnInit {
  ...
  constructor(private employeeService: EmployeeService,
    ...
    // this.brandId = auth.configportal.appId;
    //fetching doctor details from employeeService
  ...
  ngOnInit(): void {
    ...
    if (this.doctorDetails && this.doctorDetails.employeePocMappingList != undefined) {
      ...
      // console.log("poclist" + JSON.stringify(this.doctorJoinedPocList));
    }
  ...
  updateRole() {
    ...
    if (this.tempIndex != -1) {
      ...
      // this.employeeService.getServicelist();
      this.doctorDetails = this.employeeService.doctorDetail;
  ...
  // resetPocId() {
  //   this.doctorDetails.employeePocMappingList.forEach(poc => {
  //     let listedPoc = this.pocList.filter(pocl => { return poc.pocName == pocl.pocName });
  //     if (poc.pocId != listedPoc[0].pocId) {
  //       poc.pocId = listedPoc[0].pocId;
  //     }
  //   });
  // }
}