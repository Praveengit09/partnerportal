...
@Component({
  selector: 'homeorderfulfilleddiagnosticchart',
  templateUrl: './homeorderfulfilled.template.html',
  styleUrls: ['./homeorderfulfilled.style.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HomeOrderFulfilledComponent {
  nvd31Chart: any;
  nvd31Data: any;
  ...
  ngOnInit(): void {
    ...
    this.getdiagnosticcount().then(response => {
      ...
      console.log("Response In :: " + JSON.stringify(this.reportResponse));
      this.applyNvd3Data();
    }).catch((err) => {
      if (err) {
  ...
  getdiagnosticcount(): Promise<ReportResponse[]> {
    ...
    console.log("home orderss requestttt" + JSON.stringify(request));

    console.log("fromDate:: " + request.fromDate + " toDate:: " + request.toDate);
    return this.diagnosticService.getDiagnosticHomeOrderFulfilled(request);
  }
  ...
  applyNvd3Data(): void {
    ...
    response.key = "HomeOrderedFulfilled";
