...
declare var nv: any;
@Component({
    selector: 'sleepdisorder',
    templateUrl: './sleepdisorder.template.html',
    styleUrls: ['./sleepdisorder.style.scss'],
    encapsulation: ViewEncapsulation.None
})
export class SleepDisorderComponent implements OnInit {
    PHRGraphChart: any;
    isMoreItem: boolean;
    ...
    listOfQuetionId = [{ name: 'Sleep Disorders', Id: 4019 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphsleepdisorder = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphsleepdisorder.xAxis
                .axisLabel('')
                .axisLabel("Sleep Disorders")

            this.PHRGraphsleepdisorder.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartsleepdisorder svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphsleepdisorder)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphsleepdisorder.update);

            return this.PHRGraphsleepdisorder;
        });
