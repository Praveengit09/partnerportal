...
@Component({
    selector: 'epilepsy',
    templateUrl: './epilepsy.template.html',
    styleUrls: ['./epilepsy.style.scss'],
...
export class EpilepsyComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'Epilepsy', Id: 4016 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphepilepsy = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphepilepsy.xAxis
                .axisLabel('')
                .axisLabel("Epilepsy");

            this.PHRGraphepilepsy.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartepilepsy svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphepilepsy)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphepilepsy.update);

            return this.PHRGraphepilepsy;
        });
