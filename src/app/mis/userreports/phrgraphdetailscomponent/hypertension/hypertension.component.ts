...
@Component({
    ...
    templateUrl: './hypertension.template.html',
    styleUrls: ['./hypertension.style.scss'],
...
export class HypertensionComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'Hypertension ', Id: 4011 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphCharthyper = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphCharthyper.xAxis
                .axisLabel('')
                .axisLabel("Hypertension ");

            this.PHRGraphCharthyper.yAxis
                .axisLabel("Number Of Users ")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#charthyper svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphCharthyper)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphCharthyper.update);
            return this.PHRGraphCharthyper;
