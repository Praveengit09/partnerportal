...
@Component({
    selector: 'asthma',
    templateUrl: './asthma.template.html',
    styleUrls: ['./asthma.style.scss'],
...
export class AsthmaComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'Asthma', Id: 4015 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphasthma = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphasthma.xAxis
                .axisLabel('')
                .axisLabel("Asthma")

            this.PHRGraphasthma.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartasthma svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphasthma)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphasthma.update);

            return this.PHRGraphasthma;
        });
