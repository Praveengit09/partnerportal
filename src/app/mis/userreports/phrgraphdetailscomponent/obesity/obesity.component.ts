...
@Component({
    selector: 'obesity',
    templateUrl: './obesity.template.html',
    styleUrls: ['./obesity.style.scss'],
...
export class ObesityComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'Obesity', Id: 4024 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphobesity = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphobesity.xAxis
                .axisLabel('')
                .axisLabel("Obesity")

            this.PHRGraphobesity.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartobesity svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphobesity)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphobesity.update);

            return this.PHRGraphobesity;
        });
