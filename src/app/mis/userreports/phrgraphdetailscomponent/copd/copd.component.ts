...
@Component({
    selector: 'copd',
    templateUrl: './copd.template.html',
    styleUrls: ['./copd.style.scss'],
...
export class COPDComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'COPD', Id: 4014 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphcopd = nv.models.multiBarChart().reduceXTicks(true)
            ...
            this.PHRGraphcopd.xAxis
                .axisLabel('')
                .axisLabel("COPD");

            this.PHRGraphcopd.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartcopd svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphcopd)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphcopd.update);

            return this.PHRGraphcopd;
        });
