...
declare var nv: any;
@Component({
    selector: 'chronickidneydisease',
    templateUrl: './chronickidneydisease.template.html',
    styleUrls: ['./chronickidneydisease.style.scss'],
    encapsulation: ViewEncapsulation.None
})
export class ChronicKidneyDiseaseComponent implements OnInit {
    PHRGraphChart: any;
    isMoreItem: boolean;
    ...
    listOfQuetionId = [{ name: 'Chronic Kidney Disease', Id: 4017 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphchronic = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphchronic.xAxis
                .axisLabel('')
                .axisLabel("Chronic Kidney Disease");

            this.PHRGraphchronic.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartchronic svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphchronic)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphchronic.update);

            return this.PHRGraphchronic;
        });
