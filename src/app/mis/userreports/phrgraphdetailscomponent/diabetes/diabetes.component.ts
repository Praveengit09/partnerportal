...
@Component({
    selector: 'diabetes',
    templateUrl: './diabetes.template.html',
    styleUrls: ['./diabetes.style.scss'],
...
export class DiabetesComponent implements OnInit {
    ...
    listOfQuetionId = [{ name: 'Diabetes', Id: 4013 }]
    ...
    applyNvd3Data(): void {
        ...
        nv.addGraph(function () {
            this.PHRGraphdiabetes = nv.models.multiBarChart().stacked(true).showControls(false).reduceXTicks(true);
            ...
            this.PHRGraphdiabetes.xAxis
                .axisLabel('')
                .axisLabel("Diabetes");

            this.PHRGraphdiabetes.yAxis
                .axisLabel("Number of Users")
                .axisLabelDistance(-5)
                .tickFormat(d3.format(',f'));

            d3.select('#chartdiabetes svg')
                .datum(response)
                .transition().duration(500)
                .call(this.PHRGraphdiabetes)
                ;
            d3.selectAll('.nvtooltip').remove();

            nv.utils.windowResize(this.PHRGraphdiabetes.update);

            return this.PHRGraphdiabetes;
        });
